...
def load_arguments(self, _):
    ...
    with self.argument_context('storageimportexport job create') as c:
        ...
        c.argument('return_address', action=AddReturnAddress, nargs='+', help='Specifies the return address information'
                   ' for the job. Expect value: KEY1=VALUE1 KEY2=VALUE2 ... , available KEYs are: recipient-name, stree'
                   't-address1, street-address2, city, state-or-province, postal-code, country-or-region, phone, email.'
                   '')
        c.argument('return_shipping', action=AddReturnShipping, nargs='+', help='Specifies the return carrier and custo'
                   'mer\'s account with the carrier. Expect value: KEY1=VALUE1 KEY2=VALUE2 ... , available KEYs are: ca'
                   'rrier-name, carrier-account-number.')
        c.argument('shipping_information', action=AddShippingInformation, nargs='+', help='Contains information about t'
                   'he Microsoft datacenter to which the drives should be shipped. Expect value: KEY1=VALUE1 KEY2=VALUE'
                   '2 ... , available KEYs are: recipient-name, street-address1, street-address2, city, state-or-provin'
                   'ce, postal-code, country-or-region, phone.')
        c.argument('delivery_package', action=AddDeliveryPackage, nargs='+', help='Contains information about the packa'
                   'ge being shipped by the customer to the Microsoft data center. Expect value: KEY1=VALUE1 KEY2=VALUE'
                   '2 ... , available KEYs are: carrier-name, tracking-number, drive-count, ship-date.')
        c.argument('return_package', action=AddDeliveryPackage, nargs='+', help='Contains information about the package'
                   ...
                   ' yet been returned. Expect value: KEY1=VALUE1 KEY2=VALUE2 ... , available KEYs are: carrier-name, t'
                   'racking-number, drive-count, ship-date.')
        ...
        c.argument('drive_list', action=AddStorageimportexportJobCreateDriveList, nargs='+', help='List of up to ten dr'
                   ...
                   'fied for export jobs. Expect value: KEY1=VALUE1 KEY2=VALUE2 ... , available KEYs are: drive-id, bit'
                   '-locker-key, manifest-file, manifest-hash, drive-header-hash, state, copy-status, percent-complete,'
                   ' verbose-log-uri, error-log-uri, manifest-uri, bytes-succeeded.')
        ...
        c.argument('export_blob_list_blob_path', nargs='+', help='A collection of blob-path strings. Expected value: js'
                   'on-string/@json-file.')
        c.argument('export_blob_list_blob_path_prefix', nargs='+', help='A collection of blob-prefix strings. Expected '
    ...
    with self.argument_context('storageimportexport job update') as c:
        ...
        c.argument('drive_list', action=AddStorageimportexportJobUpdateDriveList, nargs='+', help='List of drives that '
                   'comprise the job. Expect value: KEY1=VALUE1 KEY2=VALUE2 ... , available KEYs are: drive-id, bit-loc'
                   'ker-key, manifest-file, manifest-hash, drive-header-hash, state, copy-status, percent-complete, ver'
                   'bose-log-uri, error-log-uri, manifest-uri, bytes-succeeded.')
