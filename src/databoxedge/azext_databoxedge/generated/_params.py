...
def load_arguments(self, _):
    ...
    with self.argument_context('databoxedge device create') as c:
        ...
        c.argument('kind', arg_type=get_enum_type(['AzureDataBoxGateway', 'AzureStackEdge', 'AzureStackHub',
                                                   'AzureModularDataCentre']), help='The etag for the devices.')
        ...
        c.argument('resource_move_details', action=AddResourceMoveDetails, nargs='+', help='The details of the move '
                   'operation on this resource.')
        c.argument('registration_id', type=str, help='Edge Subscription Registration ID', arg_group='Edge Profile '
                   'Subscription')
        c.argument('id_', options_list=['--id'], type=str, help='ARM ID of the subscription', arg_group='Edge Profile '
                   'Subscription')
        c.argument('state', arg_type=get_enum_type(['Registered', 'Warned', 'Suspended', 'Deleted', 'Unregistered']),
                   help='', arg_group='Edge Profile Subscription')
        c.argument('registration_date', type=str, help='', arg_group='Edge Profile Subscription')
        c.argument('subscription_id', type=str, help='', arg_group='Edge Profile Subscription')
        c.argument('tenant_id', type=str, help='', arg_group='Edge Profile Subscription Properties')
        c.argument('location_placement_id', type=str, help='', arg_group='Edge Profile Subscription Properties')
        c.argument('quota_id', type=str, help='', arg_group='Edge Profile Subscription Properties')
        c.argument('serialized_details', type=str, help='', arg_group='Edge Profile Subscription Properties')
        c.argument('registered_features', action=AddRegisteredFeatures, nargs='+', help='', arg_group='Edge Profile '
                   'Subscription Properties')
        c.argument('type_', options_list=['--type'], arg_type=get_enum_type(['None', 'SystemAssigned',
                                                                             'UserAssigned']), help='Identity type',
                                                                             arg_group='Identity')
    ...
    with self.argument_context('databoxedge device update') as c:
        ...
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('tags', tags_type)
        c.argument('id_', options_list=['--id'], type=str, help='The path ID that uniquely identifies the subscription '
                   'of the edge profile.', arg_group='Edge Profile Subscription')
        c.argument('type_', options_list=['--type'], arg_type=get_enum_type(['None', 'SystemAssigned',
                                                                             'UserAssigned']), help='Identity type',
                                                                             arg_group='Identity')
    ...
    with self.argument_context('databoxedge device generate-certificate') as c:
        c.argument('device_name', options_list=['--name', '-n', '--device-name'], type=str, help='The device name.',
                   id_part='name')
        c.argument('resource_group_name', resource_group_name_type)
    ...
    with self.argument_context('databoxedge device update-extended-information') as c:
        c.argument('device_name', options_list=['--name', '-n', '--device-name'], type=str, help='The device name.',
                   id_part='name')
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('client_secret_store_id', type=str, help='The Key Vault ARM Id for client secrets')
        c.argument('client_secret_store_url', type=str, help='The url to access the Client Key Vault')
        c.argument('channel_integrity_key_name', type=str, help='The name for Channel Integrity Key stored in the '
                   'Client Key Vault')
        c.argument('channel_integrity_key_version', type=str, help='The version of Channel Integrity Key stored in the '
                   'Client Key Vault')
    ...
    with self.argument_context('databoxedge order create') as c:
        ...
        c.argument('status', arg_type=get_enum_type(['Untracked', 'AwaitingFulfilment', 'AwaitingPreparation',
                                                     ...
                                                     'AwaitingDrop']), help='Status of the order as per the allowed '
                   'status types.', arg_group='Current Status')
    ...
    with self.argument_context('databoxedge addon list') as c:
        c.argument('device_name', type=str, help='The device name.')
        c.argument('role_name', type=str, help='The role name.')
        c.argument('resource_group_name', resource_group_name_type)

    with self.argument_context('databoxedge addon show') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('addon_name', options_list=['--name', '-n', '--addon-name'], type=str, help='The addon name.',
                   id_part='child_name_2')
        c.argument('resource_group_name', resource_group_name_type)

    with self.argument_context('databoxedge addon create') as c:
        c.argument('device_name', type=str, help='The device name.')
        c.argument('role_name', type=str, help='The role name.')
        c.argument('addon_name', options_list=['--name', '-n', '--addon-name'], type=str, help='The addon name.')
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('addon', type=validate_file_or_dict, help='The addon properties. Expected value: '
                   'json-string/@json-file.')

    with self.argument_context('databoxedge addon update') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('addon_name', options_list=['--name', '-n', '--addon-name'], type=str, help='The addon name.',
                   id_part='child_name_2')
    ...
    with self.argument_context('databoxedge addon delete') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('addon_name', options_list=['--name', '-n', '--addon-name'], type=str, help='The addon name.',
                   id_part='child_name_2')
    ...
    with self.argument_context('databoxedge addon wait') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('addon_name', options_list=['--name', '-n', '--addon-name'], type=str, help='The addon name.',
                   id_part='child_name_2')
    ...
    with self.argument_context('databoxedge monitoring-config list') as c:
        c.argument('device_name', type=str, help='The device name.')
        c.argument('role_name', type=str, help='The role name.')
        c.argument('resource_group_name', resource_group_name_type)

    with self.argument_context('databoxedge monitoring-config show') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('resource_group_name', resource_group_name_type)

    with self.argument_context('databoxedge monitoring-config create') as c:
        c.argument('device_name', type=str, help='The device name.')
        c.argument('role_name', type=str, help='The role name.')
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('metric_configurations', type=validate_file_or_dict, help='The metrics configuration details '
                   'Expected value: json-string/@json-file.')

    with self.argument_context('databoxedge monitoring-config update') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('metric_configurations', type=validate_file_or_dict, help='The metrics configuration details '
                   'Expected value: json-string/@json-file.')
        c.ignore('monitoring_metric_configuration')

    with self.argument_context('databoxedge monitoring-config delete') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('resource_group_name', resource_group_name_type)

    with self.argument_context('databoxedge monitoring-config wait') as c:
        c.argument('device_name', type=str, help='The device name.', id_part='name')
        c.argument('role_name', type=str, help='The role name.', id_part='child_name_1')
        c.argument('resource_group_name', resource_group_name_type)
    ...
    with self.argument_context('databoxedge trigger list') as c:
        c.argument('device_name', type=str, help='The device name.')
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('expand', type=str, help='Specify $filter=\'CustomContextTag eq <tag>\' to filter on custom context '
                   'tag property')
    ...
    with self.argument_context('databoxedge user list') as c:
        c.argument('device_name', type=str, help='The device name.')
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('expand', type=str, help='Specify $filter=\'Type eq <type>\' to filter on user type property')
