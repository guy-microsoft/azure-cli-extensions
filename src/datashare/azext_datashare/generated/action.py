...
class AddStorageAccountDataSet(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        action = self.get_action(values, option_string)
        namespace.storage_account_data_set = action
    ...
    def get_action(self, values, option_string):  # pylint: disable=no-self-use
        ...
        for k in properties:
            ...
            if kl == 'paths':
                d['paths'] = v
            elif kl == 'storage-account-resource-id':
                d['storage_account_resource_id'] = v[0]
        d['kind'] = 'StorageAccount'
        return d
...
class AddStorageAccountDataSetMapping(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        action = self.get_action(values, option_string)
        namespace.storage_account_data_set_mapping = action
